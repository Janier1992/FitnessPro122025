...
export const ChecklistWidget: React.FC<ChecklistWidgetProps> = ({ title, items: initialItems }) => {
                    ...
                    {items.map(item => (
                        <div key={item.id} className={`flex items-center p-3 rounded-lg transition-colors ${item.isCompleted ? 'opacity-50' : 'hover:bg-slate-50'}`}>
                            ...
                            {!item.isCompleted && item.actionLabel && (
                                <button className="text-xs text-brand-primary font-semibold hover:underline">{item.actionLabel}</button>
                            )}
                        </div>
                    ))}
                </div>
