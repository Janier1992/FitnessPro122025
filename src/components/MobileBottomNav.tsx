...
export const MobileBottomNav: React.FC<MobileBottomNavProps> = ({ navItems, activeItem, onNavigate }) => {
  ...
  // Logic: Show max 5 items if that's all there is.
  // If there are more than 5 items, show 4 items + More button (which makes 5 touch targets).
  const MAX_VISIBLE = 5;
  ...
  return (
    <>
        {/* Hidden Items Menu Backdrop & Container */}
        {isMenuOpen && (
            <>
                    ...
                    <div className="py-2 max-h-[60vh] overflow-y-auto">
                        {hiddenItems.map((item) => (
                            <button
                                ...
                                className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-all duration-200 hover:bg-slate-50 dark:hover:bg-slate-700/50 border-l-4 ${
        ...
        <nav className="md:hidden fixed bottom-4 left-1/2 -translate-x-1/2 w-[95%] max-w-sm bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl rounded-2xl shadow-2xl z-50 border border-slate-200 dark:border-slate-700 ring-1 ring-black/5">
            <div className="flex justify-around items-stretch h-16 px-1">
                ...
                <button
                    key={item.name}
                    onClick={() => handleNavigate(item.name)}
                    className={`flex-1 flex flex-col items-center justify-center transition-all duration-200 p-1 rounded-xl focus:outline-none active:scale-95 ${
                    activeItem === item.name
                        ? 'text-brand-primary'
